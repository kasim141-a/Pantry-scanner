...
[![Version](https://img.shields.io/badge/version-1.2.0-orange.svg)](https://github.com/kasim141-a/Pantry-scanner)
...
| Feature | Description |
|---|---|
| Pantry inventory | Track items with quantity, unit, category, storage location, and expiry date |
...
| Waste & savings stats | Track meals cooked, waste prevented, and estimated savings against a weekly budget |
...
**Main view** shows your stats, weekly budget, a **💡 Suggested for Shopping List** panel (low-stock and expiring items with per-item **🛒 Add** buttons and an **Add all** button), the expiring-soon list, categories, and the current shopping list.
...
3. **Manual entry** — type the barcode and press Submit.
...
|---|---|
| `…_total_items` | Pantry size, with category/location breakdown and `low_stock_items` attribute |
...
| Service | Purpose |
|---|---|
| `smart_pantry.add_item` / `update_item` / `remove_item` | Manage pantry items |
| `smart_pantry.scan_barcode` | Add an item by barcode (camera, scanner, ESPHome, automations) |
